...
Data Pond is under active internal development. Version `1.0.24` is the current
test build.
...
```sh
R CMD build .
R CMD INSTALL datapond_1.0.24.tar.gz
```
...
```sh
R CMD build .
R CMD check --no-manual datapond_1.0.24.tar.gz
```
